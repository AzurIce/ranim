# GPU 上传路径 Profiling 笔记（2026-09-09，分支 `upload-opt-experiments`）
...
- `benches/src/bin/profile_gpu_upload.rs`：6 场景；同时输出**内容冗余分析**
  （相邻帧 CoreItem 逐字段 diff → 理论最小上传字节）。
- render pass 级 GPU timer scope（补上了此前缺失的 begin/end scope；
  `Renderer::take_last_gpu_scopes()` 可编程读取；**运行时开关**
  `RANIM_PROFILE_GPU=1` 或面板 "GPU timers"，不再是编译期 feature）。
...
# GPU pass 时间随场景打印（benches 内部启用 GPU timers；会强制每帧
# device poll，full-frame 数字含同步开销）
```
...
# 面板里勾选 "GPU timers" 或启动时 RANIM_PROFILE_GPU=1；上传策略在面板
# "Buffer uploads" 下拉框切换，或启动时 RANIM_PROFILE_UPLOAD=skip-equal 等
```

注意：GPU timers 开启时每帧渲染后会 device poll 同步一次（wgpu-profiler
取 timer 所需），preview 的 render 耗时会偏高——平时保持关闭即可。
