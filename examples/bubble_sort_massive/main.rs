...
#[scene]
struct BubbleSortMassiveScene;

impl TimelineConstructor for BubbleSortMassiveScene {
    fn construct<'t: 'r, 'r>(
        self,
        ...
        for i in (1..num).rev() {
            for j in 0..i {
                ...
                timeline.play(
                    rects[j]
                        .transform(|data| {
                            data.set_color(manim::WHITE.with_alpha(0.5))
                                .set_fill_opacity(0.5);
                        })
                        .with_duration(anim_step_duration)
                        .with_rate_func(linear)
...
fn main() {
    render_timeline(BubbleSortMassiveScene, &AppOptions::default());
}
