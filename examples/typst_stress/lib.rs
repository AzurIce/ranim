...
fn typst_behavior(r: &mut RanimScene) {
    ...
    cells
        ...
        .move_to(DVec3::ZERO);
    let _r_matrix = r.insert(cells.into_iter().flatten().collect::<Vec<_>>());
    ...
    r.insert_with(|timeline| {
        timeline
            .forward(0.5)
            .play(from.morph_to(to.clone()).with_duration(2.0))
            .forward(0.5);
    });
...
"#,
    ...
    r.insert_with(|timeline| {
        timeline
            .play(items.clone().show())
            .forward(0.5)
            .play(items.morph_to(target.clone()).with_duration(3.0))
            .forward(0.5);
    });
