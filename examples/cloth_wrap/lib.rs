...
impl IterativeEval for ClothWrap {
    ...
    fn step(&mut self, time: &Time, delta_time: &DeltaTime) {
        ...
        // `step` and projected by `sample`.
        self.ball_y = (BALL_START_Y - BALL_SPEED * time.global_secs).max(BALL_STOP_Y);
...
fn cloth_wrap(r: &mut RanimScene) {
    ...
    r.play(Iterative(ClothWrap::new(TOTAL_SECS)).with_duration(TOTAL_SECS));
}
