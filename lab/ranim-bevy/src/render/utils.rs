use ranim_core::glam::{Vec3 as RanimVec3, Vec4 as RanimVec4};

pub(crate) fn vitem_normal_from_points(points: &[RanimVec4]) -> RanimVec3 {
    if points.len() < 3 {
        return RanimVec3::Z;
    }
...
pub(crate) fn resize_vec4_by_sample(values: &[RanimVec4], target_len: usize) -> Vec<RanimVec4> {
    if target_len == 0 {
        return Vec::new();
    }
    if values.is_empty() {
        return vec![RanimVec4::ZERO; target_len];
    }
