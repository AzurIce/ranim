...
pub trait MorphAnim: MorphRequirement + Sized + 'static {
    ...
    fn morph<F: Fn(&mut Self)>(&mut self, f: F) -> AnimationCell<Self, Morph<Self>>;
    /// Create a [`Morph`] anim from src.
    fn morph_from(&mut self, src: Self) -> AnimationCell<Self, Morph<Self>>;
    /// Create a [`Morph`] anim to dst.
    fn morph_to(&mut self, dst: Self) -> AnimationCell<Self, Morph<Self>>;
}
// ANCHOR_END: MorphAnim

// ANCHOR: MorphAnim-Impl
impl<T: MorphRequirement + 'static> MorphAnim for T {
    fn morph<F: Fn(&mut T)>(&mut self, f: F) -> AnimationCell<T, Morph<T>> {
    ...
    fn morph_from(&mut self, s: T) -> AnimationCell<T, Morph<T>> {
        Morph::new(s, self.clone())
            .into_animation_cell()
            .with_rate_func(smooth)
            .apply_to(self)
    }
    fn morph_to(&mut self, d: T) -> AnimationCell<T, Morph<T>> {
