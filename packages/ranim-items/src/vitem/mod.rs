...
pub trait VPath: Any + Clone {
    /// The normalized normal vector
    fn normal(&self) -> DVec3;
    /// Build the vector points represented by this path.
...
impl<T: VPath> Aabb for VItem<T> {
    fn aabb(&self) -> [DVec3; 2] {
        self.inner.build_vpoint_vec().aabb()
    }
}
...
impl<T: VPath> From<VItem<T>> for ranim_core::core_item::vitem::VItem {
    fn from(value: VItem<T>) -> Self {
        let normal = value.inner.normal();
        let points = value.inner.build_vpoint_vec();
        Self {
            normal,
            points,
