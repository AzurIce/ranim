...
impl RenderBindGroup {
    pub fn bind_group_layout(ctx: &WgpuContext) -> wgpu::BindGroupLayout {
        ctx.device
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("NVItem Render Bind Group Layout"),
                entries: &[
    ...
    fn new_bind_group(
        ...
        ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("NVItem Render Bind Group"),
...
pub struct NVItemPipeline {
    pipeline: wgpu::RenderPipeline,
}

impl Deref for NVItemPipeline {
...
impl RenderResource for NVItemPipeline {
    fn new(wgpu_ctx: &WgpuContext) -> Self {
        let WgpuContext { device, .. } = wgpu_ctx;

        let module = &device.create_shader_module(wgpu::include_wgsl!("./shaders/nvitem.wgsl"));

        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("NVItem Pipeline Layout"),
            bind_group_layouts: &[
